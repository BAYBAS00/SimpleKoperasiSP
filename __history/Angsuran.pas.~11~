...
procedure TFAngsuran.btnKeluarClick(Sender: TObject);
begin
Close;
end;
...
procedure TFAngsuran.edCariChange(Sender: TObject);
begin
  QAngsuran.MacroByName('WHERE').Value :=
    'WHERE a.kode_angsuran LIKE ' + QuotedStr('%' + edCari.Text + '%') +
    ' OR ag.nama LIKE ' + QuotedStr('%' + edCari.Text + '%');

  if QAngsuran.Active then
    QAngsuran.Refresh
  else
    QAngsuran.Open;
end;
